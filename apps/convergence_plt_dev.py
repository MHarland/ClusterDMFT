...
for fname in sys.argv[2:]:
    ...
    analysis.calc_distances([4990,5010])
    #analysis.calc_standard_deviations([4900, 5100])
    ...
    for blockname, block in y[0].items():
        for i, j in itt.product(*[range(block.shape[0])]*2):
            ...
            ax2.semilogy(x, [yy[blockname][i, j].imag for yy in y], label = blockname+str(i)+str(j), color = colors[c])
            c += 1
    ax1.set_xlabel("$\mathrm{loop}$")
    ax1.set_ylabel("$\Re\Delta f$")
    ax1.plot([0, len(y)],[0, 0], ls = '--', zorder = .1, color = 'gray')
    #ax2.plot([0, len(y)],[0, 0], ls = '--', zorder = .1, color = 'gray')
